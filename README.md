...
### 배포 테스트 간 주의 사항
1. 윈도우 환경의 경우, wsl을 통한 ubuntu 가상환경으로 진입해야 배포 스크립트가 동작함. 즉 wsl ubuntu 설치 필요
2. wsl로 실행할 경우 마운트 된 정확한 폴더 위치를 찾아와야 함. 따라서 intellij에서 로컬 터미널 실행 후 wsl 진입을 권장.
3. 스크립트 실행 후 애플리케이션 완전 실행까지 시간이 걸림. app.log에 프로그램 실행 과정이 로그로 찍히니 해당 파일을 참고할 것.